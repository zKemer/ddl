  ...
  if(!message.guild || message.author.bot) return
	if (message.content === `!site`) {
	  return message.channel.send(new Discord.MessageEmbed()
                                ...
                                .setDescription("Websitemiz için **[buraya](https://www.codelist.ml)** tıkla!"));
	}
  });
...
client.user.setPresence({ activity: { type: 'WATCHING', name: 'www.codelist.ml' }, status: "dnd" });
})
...
let map = ""
if(filter.length <= 0) {
map = `Sonuç Bulunamadı.`
}else{
filter.map(a => {
   
map += `${a.codeName} **->** [Tıkla](https://www.codelist.ml/code/${a.code})\n`
})
}
const aramasonucu = new Discord.MessageEmbed()
.setDescription(`Arama sonuçları \`${message.content}\`\n\n${map}`)
.setColor("BLUE")
.setFooter(config.footer)
...
client.channels.cache.get(channels.votes).send("Bu ay için bütün bot oyları silindi.")
